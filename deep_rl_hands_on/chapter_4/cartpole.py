...
def filter_batch(
...
) -> tt.List[tt.Tuple[torch.FloatTensor, torch.LongTensor]]:
    rewards = list(map(lambda e: e.reward, batch))
    reward_lower_bound = float(np.percentile(rewards, percentile))
    reward_mean = float(np.mean(rewards))

    train_obs: tt.List[np.array] = []
    train_act: tt.List[int] = []
    for episode in batch:
        if episode.reward < reward_lower_bound:
            continue
    ...
    return train_obs_v, train_act_v, reward_lower_bound, reward_mean
...
if __name__ == "__main__":
    ...
    for iter_no, batch in enumerate(iterate_batches(env, net, BATCH_SIZE)):
        ...
        optimizer.step()
        print(
            f"{iter_no}: loss={loss_v.item():.3f}, reward_mean={reward_m:.1f}, rw_bound={reward_b:.1f}"
        )
